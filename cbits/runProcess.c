...
static size_t raw_strlen(const char *str) {
  // Avoid libc in the child after fork; keep to async-signal-safe operations.
  const char *ptr = str;
  while (*ptr) {
    ptr++;
  }
...
static const char *raw_strchr(const char *str, int ch) {
  const char *ptr = str;
  while (*ptr) {
    if (*ptr == (char)ch) {
      return ptr;
...
static const char *get_path_from_env(char *const envp[]) {
  char *const *entry = envp;
  if (!entry) {
    return NULL;
  }
  while (*entry) {
    const char *val = *entry;
    if (val[0] == 'P' && val[1] == 'A' && val[2] == 'T' && val[3] == 'H' &&
        val[4] == '=') {
      return val + 5;
    }
    entry++;
...
static void copy_segment(char *dst, const char *src, size_t len) {
  size_t i;
  for (i = 0; i < len; i++) {
    dst[i] = src[i];
  }
...
static int raw_execvpe(char *const argv[], char *const envp[]) {
  char *file = argv[0];
  const char *path;
  const char *segment;
  size_t file_len;
  ...
  file_len = raw_strlen(file);
  saved_errno = ENOENT;
  segment = path;

  while (1) {
    const char *end = raw_strchr(segment, ':');
    size_t seg_len = end ? (size_t)(end - segment) : raw_strlen(segment);
    size_t needed = seg_len + (seg_len ? 1 : 0) + file_len + 1;
    char buf[PATH_MAX];

    if (needed <= sizeof(buf)) {
      size_t pos = 0;
      if (seg_len) {
        copy_segment(buf, segment, seg_len);
        pos = seg_len;
        buf[pos] = '/';
        pos++;
      }
      copy_segment(buf + pos, file, file_len);
      buf[pos + file_len] = '\0';
...
__attribute__((__noreturn__)) static void childFailed(int pipe, int failCode) {
  int err;
