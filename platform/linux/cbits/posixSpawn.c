...
/* ----------------------------------------------------------------------------
   ...
   POSIX spawn-based process support (Linux)
...
pid_t runInteractiveProcess(char *const args[], char *workingDirectory,
  ...
  if (fdStdIn == -1) {
    if (pipe2(fdStdInput, O_CLOEXEC) == -1) {
      *failed_doing = "posix_spawn: pipe2";
      return -1;
    }
    haveStdInPipe = 1;
  }
  if (fdStdOut == -1) {
    if (pipe2(fdStdOutput, O_CLOEXEC) == -1) {
      *failed_doing = "posix_spawn: pipe2";
      goto fail;
    }
    haveStdOutPipe = 1;
  ...
  if (fdStdErr == -1) {
    if (pipe2(fdStdError, O_CLOEXEC) == -1) {
      *failed_doing = "posix_spawn: pipe2";
      goto fail;
    }
    haveStdErrPipe = 1;
